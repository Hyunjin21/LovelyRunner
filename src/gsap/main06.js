...
const Main06 = () => {
  ...
  useEffect(()=>{
    ...
    .fromTo(textRef.current, { x:'50%' }, { x:'-20%', ease:'none', duration:10 },0)
  ...
  const WatchModel = ({ containerRef }) => {
    ...
    return (
      <primitive ref={watchRef} object={scene} material={materials.watch} scale={1} onPointerOver={handleHover} onPointerOut={handleHoverOut}/>
    );
  };
  ...
  return (
    <section ref={sectionRef} className='watch overflow-x-clip h-screen w-full' style={{ width:'100%', height:'100vh', margin:0, padding:0,position:'relative', marginTop:'150px'}}>
